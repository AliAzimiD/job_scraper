...
echo "Creating test scraper script..."
cat > vps_test_scraper.py << 'EOF'
#!/usr/bin/env python3
"""
...
sshpass -p "$VPS_PASS" scp -o StrictHostKeyChecking=no vps_test_scraper.py root@23.88.125.23:/opt/job-scraper/

echo "Running test script on VPS..."
sshpass -p "$VPS_PASS" ssh -o StrictHostKeyChecking=no root@23.88.125.23 "cd /opt/job-scraper && python3 vps_test_scraper.py"
