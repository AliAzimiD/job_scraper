...
for MIGRATION_FILE in /docker-entrypoint-migrations/0*.sql /docker-entrypoint-migrations/1*.sql; do
...
cleanup_docker() {
    ...
    if docker ps -a | grep -q "job_"; then
        ...
        for container in $(docker ps -a | grep job_ | awk '{print $1}'); do
            echo "Stopping container: $(docker inspect --format='{{.Name}}' $container 2>/dev/null || echo $container)"
            docker stop $container 2>/dev/null || true
            docker rm -f $container 2>/dev/null || true
        done
        success "Removed existing containers"
    else
        success "No existing containers to remove"
    fi
    ...
    if docker network ls | grep -q "job_scraper"; then
        docker network rm job_scraper_network 2>/dev/null || true
...
setup_database() {
    ...
    for migration_file in "${migration_files[@]}"; do
        if [ -f "migrations/$migration_file" ]; then
            echo "Applying migration: $migration_file"
            if ! docker exec job_db bash -c "PGPASSWORD=\$(cat /run/secrets/db_password) psql -U jobuser -d jobsdb -f /docker-entrypoint-migrations/$migration_file"; then
                handle_error "Failed to apply migration: $migration_file"
            fi
...
start_services() {
    ...
    if ! docker-compose up -d --build scraper; then
        handle_error "Failed to start scraper service"
    fi
...
show_info() {
    ...
    echo "- Clean up and restart: ./cleanup.sh && ./install.sh"
...
main() {
    ...
    echo "5. Clean up existing Docker resources"
    echo "6. Set up database and run migrations"
