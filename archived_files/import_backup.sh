...
# Import PostgreSQL backup to job scraper database
...
# Option 1: Clean approach - drop existing data first (DANGER!)
echo -e "${YELLOW}WARNING: This will drop all existing data in your database.${NC}"
read -p "Do you want to continue with dropping existing data? (y/n): " confirm
...
if [ $? -eq 0 ]; then
    ...
    docker exec -it job_db_dev psql -U $DB_USER -d $DB_NAME -c "SELECT COUNT(*) FROM jobs;"
else
