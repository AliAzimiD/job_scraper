...
echo "=== Copying fixed Dockerfile to server ==="
sshpass -p "$VPS_PASS" scp -o StrictHostKeyChecking=no fixed_dockerfile $VPS_USER@$VPS_IP:/opt/job-scraper/docker/Dockerfile

# Create a fixed docker-compose.monitoring.yml with no dependencies
...
echo "=== Copying fixed monitoring compose file to server ==="
...
sshpass -p "$VPS_PASS" ssh -o StrictHostKeyChecking=no $VPS_USER@$VPS_IP "cd /opt/job-scraper && docker-compose build && docker-compose up -d && docker-compose -f docker-compose.monitoring.yml up -d"
