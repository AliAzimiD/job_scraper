...
create_backup() {
    ...
    backup_dir="./pre_organization_backup_${timestamp}"
    mkdir -p "${backup_dir}"
    ...
    find . -maxdepth 1 -name "docker-compose*.yml" -exec cp {} "${backup_dir}/" \;
...
validate_dockercompose() {
    ...
    for compose_file in $(find . -name "docker-compose*.yml" ! -path "*/\.*" ! -path "./docker-compose.yml"); do
...
[ -f "fresh-install.sh" ] && mv fresh-install.sh archive/ && success "Moved fresh-install.sh to archive/"
[ -f "complete-setup.sh" ] && mv complete-setup.sh archive/ && success "Moved complete-setup.sh to archive/"
[ -f "install-fixed.sh" ] && mv install-fixed.sh archive/ && success "Moved install-fixed.sh to archive/"
[ -f "archive/install.sh.bak" ] || ([ -f "install.sh" ] && cp install.sh archive/install.sh.bak && success "Backed up install.sh to archive/")

# Move Docker Compose files (make copies to ensure running services aren't affected)
echo "Moving Docker Compose files..."
[ -f "docker-compose.dev.yml" ] && mv docker-compose.dev.yml docker/ && success "Moved docker-compose.dev.yml to docker/"
[ -f "docker-compose.minimal.yml" ] && mv docker-compose.minimal.yml docker/ && success "Moved docker-compose.minimal.yml to docker/"
[ -f "migrations/docker-compose.updated.yml" ] && cp migrations/docker-compose.updated.yml docker/ && success "Copied docker-compose.updated.yml to docker/"
[ -f "migrations/simplified-compose.yml" ] && cp migrations/simplified-compose.yml docker/ && success "Copied simplified-compose.yml to docker/"

# Move utility scripts
echo "Moving utility scripts..."
[ -f "cleanup.sh" ] && mv cleanup.sh scripts/ && ln -sf scripts/cleanup.sh . && success "Moved cleanup.sh to scripts/ and created symlink"
[ -f "run-migrations.sh" ] && mv run-migrations.sh scripts/ && ln -sf scripts/run-migrations.sh . && success "Moved run-migrations.sh to scripts/ and created symlink"
[ -f "start-db.sh" ] && mv start-db.sh scripts/ && ln -sf scripts/start-db.sh . && success "Moved start-db.sh to scripts/ and created symlink"
[ -f "setup-db.sh" ] && mv setup-db.sh scripts/ && ln -sf scripts/setup-db.sh . && success "Moved setup-db.sh to scripts/ and created symlink"
[ -f "setup-minimal.sh" ] && mv setup-minimal.sh scripts/ && ln -sf scripts/setup-minimal.sh . && success "Moved setup-minimal.sh to scripts/ and created symlink"
[ -f "docker_manage.sh" ] && mv docker_manage.sh scripts/ && success "Moved docker_manage.sh to scripts/"
...
cat > REVERT_ORGANIZATION.md << 'EOL'
